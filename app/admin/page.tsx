    ...
    <main className="quiz-page min-h-screen p-5 text-[#f8eee5] sm:p-10">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="quiz-surface mt-8 overflow-hidden rounded-[20px] border">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-black/10 text-xs uppercase tracking-[0.12em] text-[#f8eee5]/55"><tr><th className="px-6 py-4">Nome</th><th className="px-6 py-4">WhatsApp</th><th className="px-6 py-4">Resultado</th><th className="px-6 py-4">Data (Brasília)</th><th className="px-6 py-4 text-right">Ações</th></tr></thead><tbody>{responses.map((response) => <tr key={response.id} className="border-t border-[#c4946f]/15 text-[#f8eee5]/80 transition hover:bg-white/[0.03]"><td className="px-6 py-4 font-medium text-[#f8eee5]"><Link className="hover:text-[#d8af7a]" href={`/admin/responses/${response.id}`}>{response.name}</Link></td><td className="px-6 py-4">{response.phone}</td><td className="px-6 py-4">{response.level}</td><td className="whitespace-nowrap px-6 py-4">{formattedDate(response.created_at)}</td><td className="px-6 py-4 text-right"><Link href={`/admin/responses/${response.id}`} aria-label={`Ver todas as respostas de ${response.name}`} title="Ver todas as respostas" className="inline-flex items-center gap-2 whitespace-nowrap rounded-full border border-[#d8af7a]/50 px-4 py-2 text-sm font-medium text-[#d8af7a] transition hover:border-[#d8af7a] hover:bg-[#d8af7a]/10 focus:outline-none focus:ring-2 focus:ring-[#e6c18a]"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-[1.8]"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.75"/></svg><span>Ver respostas</span></Link></td></tr>)}</tbody></table></div>
