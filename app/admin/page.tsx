...
type ResponseRow = { id: number | string; name: string; email: string; phone: string; level: string; created_at: string };
...
export default async function AdminPage() {
  ...
  const [metricsRows, levelRows, responseRows] = await Promise.all([
    ...
    sql`SELECT id, name, email, phone, level, created_at FROM responses ORDER BY created_at DESC LIMIT 150`,
  ]);
