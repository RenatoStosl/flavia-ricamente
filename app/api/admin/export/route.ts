...
export async function GET() {
  ...
  const rows = await sql`SELECT id, created_at, name, email, phone, score, level, duration_seconds, utm_source, utm_medium, utm_campaign, answers FROM responses ORDER BY created_at DESC`;
  const headers = ["id", "created_at", "name", "email", "phone", "score", "level", "duration_seconds", "utm_source", "utm_medium", "utm_campaign", "answers"];
